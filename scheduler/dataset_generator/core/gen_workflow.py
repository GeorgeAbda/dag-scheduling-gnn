...
def generate_workflows(
    ...
    def req_cpu_cores_gen() -> int:
        # Sample in [1, max_req_cpu_cores], inclusive of the max
        return int(rng.randint(1, max(2, max_req_cpu_cores//4) + 1))
    ...
    def dag_gen() -> dict[int, set[int]]:
        if dag_method == "gnp":
            ...
            if gnp_p is None:
                p = None
            elif isinstance(gnp_p, tuple) and len(gnp_p) == 2:
                low, high = float(gnp_p[0]), float(gnp_p[1])
                p = float(rng.uniform(low, high))
            else:
                p = float(gnp_p)  # type: ignore[arg-type]
            return generate_dag(
                dag_method,
                gnp_min_n=gnp_min_n,
                gnp_max_n=gnp_max_n,
    ...
    for workflow_id in range(workflow_count):
        ...
        tasks: list[Task] = [
            Task(
                ...
                req_cpu_cores=req_cpu_cores_gen(),
