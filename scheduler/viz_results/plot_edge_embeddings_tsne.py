...
def extract_edge_embeddings_and_labels(agent: AblationGinAgent, args: AblArgs, rng_seed: int = 1):
    ...
    with torch.no_grad():
        ...
        # Also get action scores from the full actor (includes optional global concat)
        action_scores_mat = agent.actor(obs)  # shape: (num_tasks, num_vms)
    ...
    # Per-edge action scores aligned to compat edges order
    with torch.no_grad():
        edge_scores = action_scores_mat[obs.compatibilities[0][:E], obs.compatibilities[1][:E]].detach().cpu().numpy()
    # Compute Top-3 ranks per episode (1=top1, 2=top2, 3=top3, 0=not top3)
    top_rank = np.zeros(E, dtype=np.int32)
    k = min(3, E)
    if k > 0:
        top_idx = np.argpartition(edge_scores, -k)[-k:]
        order = top_idx[np.argsort(edge_scores[top_idx])[::-1]]
        for rank_idx, idx in enumerate(order, start=1):
            top_rank[int(idx)] = int(rank_idx)
    ...
    env.close()
    return edge_emb, vm_half, labels
...
def plot_tsne(Y: np.ndarray, labels: dict, out_dir: Path, prefix: str):
    ...
    print(f"Plotting t-SNE for {prefix} at {out_dir}")
    ...
    plt.close()
    print(f"Saved t-SNE plot for {prefix} at {out_dir / f'{prefix}_tsne_cp.png'}")
    ...
    plt.close()
    print(f"Saved t-SNE plot for {prefix} at {out_dir / f'{prefix}_tsne_deg.png'}")
    # 3) Ready mask
    plt.figure(figsize=(6,5))
    ...
    plt.close()
    print(f"Saved t-SNE plot for {prefix} at {out_dir / f'{prefix}_tsne_ready.png'}")
    # 4) VM id coloring
    plt.figure(figsize=(6,5))
    ...
    plt.close()
    print(f"Saved t-SNE plot for {prefix} at {out_dir / f'{prefix}_tsne_vm.png'}")
    # 5) Task id coloring
    plt.figure(figsize=(6,5))
    ...
    plt.close()
    print(f"Saved t-SNE plot for {prefix} at {out_dir / f'{prefix}_tsne_task.png'}")
    # 6) Top-3 actions overlay plot
    if 'top_rank' in labels:
        tr = labels['top_rank'].astype(int)
        plt.figure(figsize=(6,5))
        # Base cloud in light gray
        plt.scatter(Y[:,0], Y[:,1], c='lightgray', s=8, alpha=0.25, label='others')
        # Overlay top-1/2/3 with distinct markers/colors
        for r, marker, color in [(1, '*', 'red'), (2, 'X', 'orange'), (3, 'D', 'gold')]:
            idx = np.where(tr == r)[0]
            if idx.size > 0:
                plt.scatter(Y[idx,0], Y[idx,1], c=color, marker=marker, s=60, edgecolors='black', linewidths=0.5, label=f'Top {r}')
        plt.legend(loc='best', fontsize=8)
        plt.title(f"{prefix} — Top-3 actions in embedding space")
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(out_dir / f"{prefix}_tsne_top3.png", dpi=200)
        plt.close()
        print(f"Saved t-SNE plot for {prefix} at {out_dir / f'{prefix}_tsne_top3.png'}")
...
def main():
    ...
    for s in range(args.episodes):
        X_full, X_vm, lab = extract_edge_embeddings_and_labels(agent, eval_args, rng_seed=1_000_000_001 + s)
    ...
    labels = {
        ...
        'top_rank': np.concatenate([d.get('top_rank', np.zeros_like(d['vm'])) for d in lab_collect], axis=0),
    }
