...
def _plot_action_regions_single_task(
    ...
    try:
        ...
        for i in range(0, grid.shape[0], batch):
            ...
            with torch.no_grad():
                acts, _, _, _ = agent.get_action_and_value(st, deterministic=True)
...
def main(a: Args) -> None:
    ...
    try:
        if getattr(a, "host_specs_path", None):
            gen_vm.HOST_SPECS_PATH = Path(str(a.host_specs_path))
            print(f"[eval] Overriding HOST_SPECS_PATH to: {gen_vm.HOST_SPECS_PATH}")
    except Exception as e:
    ...
    # We fix a script-level seed so that repeated invocations with the same
    # configs and checkpoints are bitwise-reproducible.
    eval_seed = 12345
    try:
