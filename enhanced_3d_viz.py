...
def create_enhanced_3d_surface(grids, feas_mask, visited, side, dag_type, metric, out_dir, 
    ...
    # Apply smoothing for better visual quality over the feasible region.
    # We only smooth where we have data; non-feasible cells remain NaN so the
    # surface is visually focused on the feasible landscape.
    Z_smooth = Z_plot.copy()
    valid_mask = ~np.isnan(Z_plot)
    if np.any(valid_mask):
        try:
            from scipy.ndimage import gaussian_filter
            # Smooth a temporary array and then project back onto valid cells.
            temp = Z_plot.copy()
            temp[np.isnan(temp)] = 0  # Temporarily fill NaNs
            # Slightly stronger blur than before to reduce spiky appearance.
            smoothed = gaussian_filter(temp, sigma=0.8)
    ...
    # Add a translucent base plane under the feasible region to visually
    # separate feasible cells from the empty/non-feasible part of the grid.
    if np.any(~np.isnan(Z_smooth)):
        z_base = float(np.nanmin(Z_smooth)) - 0.05
        Z_base = np.full_like(Z_smooth, np.nan)
        Z_base[feas_crop & ~np.isnan(Z_smooth)] = z_base
        ax.plot_surface(
            X, Y, Z_base,
            color='#d0e1ff',
            alpha=0.45,
...
def create_enhanced_side_by_side_3d(a, sf_limit, metric, out_dir):
    ...
    create_difference_3d_surface(grids_lin, feas_lin, visited_lin, 
                                grids_gnp, feas_gnp, visited_gnp, side_gnp, 
                                metric, out_dir, label_a="longcp", label_b="wide")
...
def create_difference_3d_surface(
    grids_a,
    feas_a,
    visited_a,
    grids_b,
    ...
    """Create a 3D surface of metric differences over the common feasible region.

    X/Y follow the same Hilbert grid indexing as the individual landscapes.
    Z encodes normalized |metric_a - metric_b|, while color encodes the signed
    difference (metric_a - metric_b) using a diverging colormap.
    """

    G_a = grids_a[metric]
    G_b = grids_b[metric]

    # Restrict to cells that are feasible and visited in both domains.
    common = feas_a & feas_b & visited_a & visited_b
    if not np.any(common):
        print("[diff3d] No common feasible visited cells; skipping difference surface.")
        return None, None

    D = np.full_like(G_a, np.nan, dtype=float)
    D[common] = G_a[common] - G_b[common]

    valid = np.isfinite(D)
    if not np.any(valid):
        print("[diff3d] No finite differences on common feasible cells; skipping.")
        return None, None

    max_abs = float(np.nanmax(np.abs(D[valid])))
    if max_abs <= 0:
        print("[diff3d] All differences are zero; skipping difference surface.")
        return None, None

    # Z height = normalized magnitude of the difference
    Z = np.full_like(D, np.nan, dtype=float)
    Z[valid] = np.abs(D[valid]) / max_abs

    H, W = Z.shape
    idx = np.argwhere(common)
    if idx.size > 0:
        pad = 3
        iy_min = max(0, int(idx[:, 0].min()) - pad)
        iy_max = min(H - 1, int(idx[:, 0].max()) + pad)
        ix_min = max(0, int(idx[:, 1].min()) - pad)
        ix_max = min(W - 1, int(idx[:, 1].max()) + pad)
    else:
        iy_min, iy_max, ix_min, ix_max = 0, H - 1, 0, W - 1
    ...
    X, Y = np.meshgrid(x_range, y_range)
    Z_plot = Z[np.ix_(y_range, x_range)]
    D_plot = D[np.ix_(y_range, x_range)]
    common_plot = common[np.ix_(y_range, x_range)]

    Z_plot[~common_plot] = np.nan
    D_plot[~common_plot] = np.nan
    ...
    norm_div = Normalize(vmin=-max_abs, vmax=max_abs)
    cmap_div = _cm.get_cmap('coolwarm')
    facecols = cmap_div(norm_div(D_plot))
    ...
    title = f"Difference surface: {label_a} - {label_b} ({metric})"
    ax.set_title(title, fontsize=14, fontweight='bold', pad=20, color='#2c3e50')
    ax.set_xlabel('Solution Space X', fontsize=10, labelpad=8, color='#34495e')
    ax.set_ylabel('Solution Space Y', fontsize=10, labelpad=8, color='#34495e')
    ax.set_zlabel('Normalized |Δ|', fontsize=10, labelpad=8, color='#34495e')

    from matplotlib.cm import ScalarMappable

    sm = ScalarMappable(norm=norm_div, cmap=cmap_div)
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, shrink=0.6, aspect=25, pad=0.1)
    cbar.set_label(f'{metric} difference ({label_a} - {label_b})', fontsize=11, fontweight='bold',
                   color='#2c3e50', labelpad=15)
    cbar.ax.tick_params(labelsize=9, colors='#34495e', width=0.8, length=3)

    common_count = int(np.sum(common))
    visited_union = int(np.sum(visited_a | visited_b))
    stats_text = (
        f"Common feasible visited: {common_count:,}\n"
        f"Visited union: {visited_union:,}\n"
        f"Max |Δ|: {max_abs:.3f}"
    )
    ax.text2D(
    ...
    os.makedirs(out_dir, exist_ok=True)
    base_name = f"enhanced_3d_diff_{label_a}_minus_{label_b}_{metric}"
    png_path = os.path.join(out_dir, f"{base_name}.png")
    pdf_path = os.path.join(out_dir, f"{base_name}.pdf")
    ...
    plt.close(fig)
    print(f"Difference 3D surface saved: {png_path}")
    return png_path, pdf_path
