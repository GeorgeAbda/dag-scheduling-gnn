#!/usr/bin/env python3
"""
Simplified adversarial attacks and defenses demonstration.

This script demonstrates the core functionality of the adversarial framework
with a simpler approach that doesn't require complex observation conversion.
"""
...
def create_simple_test_observation():
    """Create a simple test observation for demonstration."""
    num_tasks = 5
    num_vms = 3
...
def create_mock_agent():
    ...
    class MockAgent(torch.nn.Module):
        ...
        def forward(self, obs):
            ...
            if hasattr(obs, 'to_tensor'):
                features = obs.to_tensor()
            elif hasattr(obs, 'task_length'):  # GinAgentObsTensor
                # Create a simple feature vector from the observation
                num_tasks = obs.task_length.shape[0]
                num_vms = obs.vm_speed.shape[0]
                features = torch.rand(num_tasks + num_vms, 16)  # Mock features
            else:
                features = obs
...
def demonstrate_attacks():
    ...
    # Create test data
    clean_obs = create_simple_test_observation()
...
def main():
    ...
    print("=" * 60)
    print("Adversarial Attacks and Defenses Demonstration")
    print("=" * 60)
    ...
    try:
        ...
        # Summary
        print("\nSUMMARY:")
        print("- Successfully demonstrated 5 different attack types")
