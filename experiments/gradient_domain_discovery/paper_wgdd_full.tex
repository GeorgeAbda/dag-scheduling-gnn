...
\acmConference[ICML '25]{International Conference on Machine Learning}{July 2025}{Vienna, Austria}
...
\ccsdesc[300]{Computing methodologies~Multi-task learning}

\keywords{Multi-Objective RL, Domain Discovery, Wasserstein Distance, Gradient Analysis, Unsupervised Learning}
...
Multi-objective reinforcement learning (MORL) addresses sequential decision problems where agents must simultaneously optimize multiple, often conflicting, objectives \cite{roijers2013survey}. In practice, the trade-off structure between objectives varies across environment configurations: some configurations admit aligned objectives where a single strategy optimizes all goals, while others exhibit fundamental conflicts requiring careful prioritization.
...
\begin{enumerate}
    \item \textbf{Gradient Distribution Characterization}: We show that a generalist policy's gradient responses to different objective weightings encode sufficient information to distinguish environment configurations with different trade-off structures.
    
    \item \textbf{Wasserstein-Based Similarity}: We introduce a principled distance metric between configurations based on the Wasserstein distance between their gradient distributions, capturing distributional differences that point estimates miss.
    
    \item \textbf{Automatic Domain Selection}: We provide a complete pipeline including multi-scale objective sampling, spectral clustering, automatic $k$ selection via silhouette analysis, and bootstrap stability validation.
    
    \item \textbf{Empirical Validation}: On a cloud scheduling benchmark, WGDD achieves perfect domain recovery (NMI=1.0) without access to ground-truth labels, correctly identifying the partition between wide-parallel and long-sequential workflows.
\end{enumerate}
...
MORL extends standard RL to vector-valued rewards \cite{roijers2013survey, hayes2022practical}. Approaches include scalarization methods that convert to single-objective problems \cite{van2014multi}, Pareto-based methods that maintain solution sets \cite{van2013scalarized}, and preference-conditioned policies that adapt to runtime weightings \cite{abels2019dynamic}. Our work complements these by addressing the orthogonal problem of identifying when different environment configurations require different strategies.

\paragraph{Multi-Task Learning and Gradient Conflicts.}
Multi-task learning (MTL) trains shared representations across related tasks \cite{caruana1997multitask}. Recent work identifies gradient conflicts as a key challenge: when task gradients oppose, naive averaging degrades performance. PCGrad \cite{yu2020gradient} projects conflicting gradients, CAGrad \cite{liu2021conflict} finds conflict-averse directions, and MGDA \cite{sener2018multi} computes Pareto-optimal gradient combinations. These methods assume known task boundaries. We address the complementary problem of discovering task (domain) boundaries from gradient patterns.

\paragraph{Task Affinity and Clustering.}
Fifty et al. \cite{fifty2021efficiently} measure task affinity via gradient similarity to group related tasks. Standley et al. \cite{standley2020tasks} use transfer learning performance for task grouping. Unlike these approaches that require training multiple models or known task labels, WGDD operates with a single generalist and discovers structure from gradient distributions alone.

\paragraph{Domain Adaptation and Discovery.}
Domain adaptation addresses distribution shift between source and target domains \cite{ben2010theory}. Domain discovery methods cluster unlabeled data to identify latent domains \cite{hoffman2012discovering}. In RL, domain randomization \cite{tobin2017domain} and system identification \cite{yu2017preparing} handle environment variation. WGDD differs by discovering domains based on \emph{objective trade-off structure} rather than observation distributions.

\paragraph{Optimal Transport in Machine Learning.}
Wasserstein distances from optimal transport theory provide principled distribution comparisons \cite{villani2008optimal}. Applications include generative modeling \cite{arjovsky2017wasserstein}, domain adaptation \cite{courty2017optimal}, and representation learning \cite{kolouri2017optimal}. We apply Wasserstein distances to policy gradient distributions, a novel application enabling gradient-based domain discovery.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/wgdd_pipeline.png}
...
Using PPO \cite{schulman2017proximal} with a balanced scalarization ($\alpha = 0.5$):
\[
...
Apply normalized spectral clustering \cite{ng2001spectral} to the affinity matrix, computing eigenvectors of the normalized Laplacian and applying $k$-means in the spectral embedding.
...
\begin{proposition}[Gradient Subspace]
...
We use a heterogeneous graph neural network (GNN) with:
\begin{itemize}
    \item 2-layer GIN backbone
    \item 128-dimensional hidden layers
    \item 64-dimensional node embeddings
    \item BatchNorm and task dependency edges
\end{itemize}
Total parameters: 213,634.
...
\begin{itemize}
    \item 40 MDPs (20 Wide, 20 LongCP)
    \item 20 objectives (2 extreme + 10 uniform + 8 focused)
    \item 5 gradient replicates per (MDP, objective)
    \item 256 rollout steps per gradient
    \item 20 bootstrap iterations
\end{itemize}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/k_selection.png}
...
Figure~\ref{fig:distance-matrix} shows the Wasserstein distance matrix, revealing clear block structure corresponding to the two domains.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/wgdd_main_results.png}
    \caption{WGDD analysis: (a) Wasserstein distance matrix with block structure, (b) clusters vs. edge probability, (c) $k$-selection curve, (d) Pareto alignment scores.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/wgdd_embedding.png}
    \caption{t-SNE embedding of Wasserstein distances. Left: predicted clusters. Right: true labels.}
    \label{fig:embedding}
\end{figure}

\subsection{Conflict Analysis}

Figure~\ref{fig:conflict} shows the gradient conflict tensor analysis. The conflict between pure makespan and energy gradients varies across configurations:
\begin{itemize}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/wgdd_conflict_analysis.png}
    \caption{Conflict analysis: (a) Mean conflict by edge probability, (b) Conflict by cluster, (c) Bootstrap co-occurrence matrix.}
    \label{fig:conflict}
\end{figure}
...
WGDD demonstrates that gradient distributions encode sufficient information for unsupervised domain discovery, opening new avenues for curriculum design, mixture-of-experts deployment, and interpretable MORL. We release our code and benchmark at \url{https://anonymous.github.io/wgdd}.

%% ============================================================================
%% ACKNOWLEDGMENTS
%% ============================================================================
\begin{acks}
The authors thank the anonymous reviewers for their valuable feedback. This work was supported by [anonymized].
\end{acks}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/gradient_subspace_pca_HETERO.png}
    \caption{PCA projection of gradient distributions. Left: colored by $\alpha_{\text{makespan}}$. Right: per-MDP means colored by domain.}
